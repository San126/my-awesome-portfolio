...
<div class="relative flex flex-col max-w-[1400px] mx-auto w-full text-sm sm:text-base min-h-screen">
  <div class={"fixed bottom-0 w-full duration-200 flex p-10 z-[10]" +
    y > 0 ? 'opacity-full pointer-evnets-auto' : 'pointer-events-none opacity-0'
}>
    <button on:click={goTop} class="ml-auto rounded-full aspect-square bg-slate-900 text-sky-400 px-5 sm:px-4 hover:bg-slate-800 cursor-pointer">
